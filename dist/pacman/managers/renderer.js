...
export class Renderer {
    ...
    renderPellets(maze) {
        ...
        for (const pos of powerPelletPositions) {
            const pixelPos = MathUtils.gridToPixel(pos.x, pos.y, gridSize);
            this.ctx.beginPath();
            this.ctx.arc(pixelPos.x, pixelPos.y, GAME_CONFIG.POWER_PELLET_RADIUS * pulseScale, 0, 2 * Math.PI);
            this.ctx.fill();
        }
        this.ctx.restore();
    }
     ...
     */
    renderOverlay(title, message, showButton = false) {
     ...
     */
    renderText(text, x, y, fontSize = 20, color = GAME_CONFIG.COLORS.TEXT, font = 'Arial') {
        this.ctx.save();
     ...
     */
    renderCenteredText(text, y, fontSize = 20, color = GAME_CONFIG.COLORS.TEXT) {
