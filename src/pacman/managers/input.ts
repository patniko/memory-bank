...
export class InputManager {
    ...
    private touchStartPos: { x: number; y: number } | null;
    private touchMinDistance: number;
        ...
        controlButtons.forEach(button => {
            ...
            if (direction) {
                ...
                button.addEventListener('touchstart', (event) => {
                    ...
                    this.setDirectionInput(direction as Direction, true);
                });
                
                button.addEventListener('touchend', (event) => {
                    event.preventDefault();
                    this.setDirectionInput(direction as Direction, false);
                });
    ...
    private handleKeyDown(event: KeyboardEvent): void {
        ...
        if (this.keysPressed.has(key)) return; // Prevent key repeat
        this.keysPressed.add(key);

        // Update input state
        if ((INPUT_KEYS.UP as readonly string[]).includes(key)) {
            this.inputState.up = true;
        } else if ((INPUT_KEYS.DOWN as readonly string[]).includes(key)) {
            this.inputState.down = true;
        } else if ((INPUT_KEYS.LEFT as readonly string[]).includes(key)) {
            this.inputState.left = true;
        } else if ((INPUT_KEYS.RIGHT as readonly string[]).includes(key)) {
            this.inputState.right = true;
        } else if ((INPUT_KEYS.PAUSE as readonly string[]).includes(key)) {
            this.triggerCallback('pause');
        } else if ((INPUT_KEYS.START as readonly string[]).includes(key)) {
            this.triggerCallback('start');
        }
     ...
     */
    private setDirectionInput(direction: Direction, pressed: boolean): void {
        switch (direction) {
            case Direction.UP:
    ...
    private isGameKey(key: string): boolean {
        const allKeys = [
            ...(INPUT_KEYS.UP as readonly string[]),
            ...(INPUT_KEYS.DOWN as readonly string[]),
            ...(INPUT_KEYS.LEFT as readonly string[]),
            ...(INPUT_KEYS.RIGHT as readonly string[]),
            ...(INPUT_KEYS.PAUSE as readonly string[]),
            ...(INPUT_KEYS.START as readonly string[])
        ];
     ...
     */
    private triggerDirectionChange(direction: Direction): void {
